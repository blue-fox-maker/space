...
public:
  ...
  [[nodiscard]] constexpr index_type find(index_type x) const noexcept { return x == _parents[x]? x:(_parents[x]=find(_parents[x]));}
  [[nodiscard]] constexpr index_type find(index_type x) noexcept { return x == _parents[x]? x: find(_parents[x]);}
  ...
  constexpr bool merge(index_type x, index_type y) noexcept {
    ...
    // if(_ranks[x]==_ranks[y] && x!=y) _ranks[y]++;
    if(x==y) return false;

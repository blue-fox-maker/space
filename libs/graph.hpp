...
//   auto query(index_type lower, index_type upper) {
//     auto etf = eular_tour_forest<index_type>{num_vert};
//     // auto uiter = std::ranges::upper_bound(index, lower, {}, [](const auto &x) { return std::get<2>(x); });
//     // for (auto iter = index.begin(); iter !=uiter ; iter++) {
//     //   auto [u, v, _, ta, tb] = *iter;
//     //   if (lower <= ta && tb <= upper) {
//     //     etf.merge(u, v);
//     //   }
//     // }
//     for(auto [u,v,t,ta,tb]: index){
